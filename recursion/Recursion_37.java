...
public class Recursion_37 {
    private static int findAnswer(int a, int b, int c, int n){
        if(n == 1) return a;
        if(n == 2) return b;
        if(n == 3) return c;

        return findAnswer(a, b, c, n-1) + findAnswer(a, b, c, n-2) + findAnswer(a, b, c, n-3);
    }
    public static void main(String[] args){
        System.out.println(findAnswer(1, 3, 2, 6));
    }
}
